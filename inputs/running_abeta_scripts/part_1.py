...
import numpy as np
seq = 'AEEE'#sys.argv[1]
name = 'playing' #sys.argv[2]
debug = False
pickle_name = name + '.pickle'
MPI_NP = 2
peptide_copies=1#int(sys.argv[3])
...
atoms_in_chain=int(int(total_no_atoms)/int(peptide_copies))
        ...
        with open(output_pdbfile,'w') as f:
            ...
            for i in np.arange(number_of_chains):           #iterates through the copies of chains  
                for j in np.arange(atoms_in_chain):         #itarates through the atoms in the chain
                    ...
                    a=lines[i*(atoms_in_chain)+i+j]             #reads the old pdbfile info pertaining to the atoms of interest  
                    ...
                    if(j==int(atoms_in_chain-1)):   #print int(i*atoms_in_chain+j),int((atoms_in_chain*i+atoms_in_chain-1))
                    #    print "wrote TER"
...
file3='template.pdb'
new_pdb_file=pdbfile_generator_w_chain_id(int(peptide_copies),int(atoms_in_chain),5,'new_pdb.pdb',file0)
...
with open(ps.pickle_name, 'w') as f:
    pickle.dump(ps, file=f)
ps.run(mdpfile='peptidesim_anneal.mdp',tag='annealing',mdp_kwargs={'nsteps':int(5* 5*10**2)},run_kwargs={'cpt':5},mpi_np=MPI_NP)#change the time step to 2 ns
#ps.run(mdpfile='peptidesim_anneal.mdp',tag='annealing',mdp_kwargs={'nsteps':int(2000* 5*10**2)},run_kwargs={'cpt':5},mpi_np=MPI_NP)#change the time step to 2 ns
...
ps.run(mdpfile='peptidesim_npt.mdp', tag='equil_npt', mdp_kwargs={'nsteps': int(5 * 5*10**2),'ref_t':278},run_kwargs={'cpt':5},mpi_np=MPI_NP)
