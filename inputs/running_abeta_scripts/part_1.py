...
new_pdb_file = pdbfile_generator_w_chain_id(
    int(peptide_copies), int(atoms_in_chain), 5, 'new_pdb.pdb', file0)
...
def data_folder(number_amino_acids, name, copies_chains):
    ...
    if (name != 'HAshifts.dat' and name != 'Hshifts.dat'):
        ...
        with open(new_file, 'w') as f:  # creates a new file to copy over the chemical shufts
            ...
            number_chains = number_amino_acids / copies_chains
            for i in range(
                    copies_chains):  # iterates throug a number of copies
