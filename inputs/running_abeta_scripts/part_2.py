...
pte_result = ps.pte_replica(mpi_np=MPI_NP, max_tries=2,min_iters=1,
                              mdp_kwargs={'nsteps':int(pte_tune_time* 5*10**5)}, replicas=replicas,hills_file_location=os.getcwd(),run_kwargs={'cpt':cpt_time},
                              hot=279,hill_height=1,sigma=500,bias_factor=20,eff_threshold=replica_eff_threshold,cold=278,exchange_period=2)
...
ps.run(mdpfile='peptidesim_nvt.mdp', tag='nvt_conver_eds_{}'.format(2),  mdp_kwargs=kwargs,run_kwargs={'plumed':'plumed_eds_conver_pt_wte_metad.dat', 'replex': remd_exchange, 'cpt':cpt_time},mpi_np=MPI_NP)
with open(ps.pickle_name, 'wb') as f:
    pickle.dump(ps, file=f)
